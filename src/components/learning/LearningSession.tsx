...
import type { GetLearningSessionResponseDTO, LearningSessionCardDTO, SessionProgressDTO } from "../../types";
...
export function LearningSession({ onEnd }: LearningSessionProps) {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center">
        <div className="bg-green-50 border border-green-200 rounded-lg p-8">
          ...
          <h2 className="text-2xl font-bold text-green-800 mb-4">Sesja nauki zakończona!</h2>

          {sessionInfo && (
            <div className="text-green-700 mb-6 space-y-2">
              <div>
                Przejrzano fiszek: <strong>{sessionInfo.cards_reviewed}</strong>
              </div>
              <div>
                Czas sesji: <strong>{formatSessionTime()}</strong>
              </div>
          ...
          <div className="space-x-3">
            <Button onClick={() => loadNextCard()}>Rozpocznij nową sesję</Button>
            <Button onClick={handleEndSession} variant="outline">
              Powrót do fiszek
            </Button>
          </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Learning card */}
      <LearningCard card={currentCard} onRate={handleRateCard} isLoading={rating} />
    </div>
  );
}
