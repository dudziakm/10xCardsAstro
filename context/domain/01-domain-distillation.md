...
| `src/components/learning/**` | język UI i faktyczny user flow | zachowanie aktualne |
| `src/lib/services/learning.service.ts` | reguły wykonawcze/persistence | zachowanie aktualne |
| `supabase/migrations/*learning*` | constraints i authorization | wersjonowany model, remote unknown |

## Ubiquitous Language

| Termin domenowy | Definicja robocza | Obecny odpowiednik |
|---|---|---|
| Fiszka (`Flashcard`) | treść z przodem i tyłem należąca do użytkownika | `flashcards` / `LearningSessionCardDTO` |
| Sesja nauki (`Learning Session`) | aktywny okres, w którym użytkownik otrzymuje i ocenia karty | `learning_sessions` |
| Przedstawienie karty (`Card Presentation`) | fakt, że konkretna karta została wydana do konkretnej sesji | brak modelu |
| Odsłonięcie (`Reveal`) | lokalna akcja pokazująca tył; warunek UX przed ratingiem | stan `isFlipped` w `LearningCard` |
| Rating | samoocena 1..5 po reveal | Zod + `rateFlashcard` |
| Powtórka (`Review`) | zaakceptowany rating karty przedstawionej w sesji | mylony z GET/incrementem |
| Postęp karty (`Flashcard Progress`) | liczba powtórek, trudność i następny termin | `flashcard_progress` |
| Termin powtórki (`Due Date`) | moment, od którego karta kwalifikuje się do sesji | `next_review_date` |
| Harmonogram (`Review Schedule`) | wynik ratingu: nowa trudność i termin | `ReviewScheduler` |
| Karta należna (`Due Card`) | brak progress/terminu lub termin ≤ now | filtr w `getNextCard` |
| Przejrzane karty (`Cards Reviewed`) | liczba zakończonych ratingiem powtórek w sesji | kod liczy prezentacje, więc termin rozjechany |
