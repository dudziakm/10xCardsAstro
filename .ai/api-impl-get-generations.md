...
```typescript
// Typy z src/types.ts
import type { GenerationListItemDTO, GenerationListResponseDTO, PaginationDTO } from "../types";
...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!result.success) {
      return new Response(JSON.stringify({ error: "Invalid parameters", details: result.error.format() }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    // Zwróć odpowiedź
    return new Response(JSON.stringify(generationsData), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (error) {
