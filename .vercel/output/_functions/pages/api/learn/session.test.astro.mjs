...
describe("Learning Session API Integration Tests", () => {
  ...
  describe("GET /api/learn/session", () => {
    ...
    it("should handle invalid session error", async () => {
      ...
      await expect(mockLearningService.getNextCard("test-user-123", "invalid-session")).rejects.toThrow(
  ...
  describe("POST /api/learn/session/rate", () => {
    it("should rate flashcard and update learning progress", async () => {
      ...
      const result = await mockLearningService.rateFlashcard("test-user-123", "session-123", "card-123", 4);
      ...
      expect(mockLearningService.rateFlashcard).toHaveBeenCalledWith("test-user-123", "session-123", "card-123", 4);
    });
    it("should validate rating values", async () => {
      ...
      await expect(mockLearningService.rateFlashcard("user-123", "session-123", "card-123", 0)).rejects.toThrow(
        "Invalid rating: must be between 1 and 5"
      );
      await expect(mockLearningService.rateFlashcard("user-123", "session-123", "card-123", 6)).rejects.toThrow(
        "Invalid rating: must be between 1 and 5"
      );
    });
    it("should handle flashcard not found error", async () => {
      ...
      await expect(
        mockLearningService.rateFlashcard("test-user-123", "session-123", "nonexistent-card", 3)
    ...
    it("should verify session ownership", async () => {
      ...
      await expect(
        mockLearningService.rateFlashcard("test-user-123", "other-user-session", "card-123", 3)
  ...
  describe("Spaced Repetition Algorithm", () => {
    it("should calculate appropriate intervals for different ratings", async () => {
      const testCases = [
        { rating: 1, expectedMinDays: 1, expectedMaxDays: 2 },
        // Again - short interval
        { rating: 2, expectedMinDays: 1, expectedMaxDays: 3 },
        // Hard - short interval
        { rating: 3, expectedMinDays: 3, expectedMaxDays: 5 },
        // Good - medium interval
        { rating: 4, expectedMinDays: 6, expectedMaxDays: 8 },
        // Easy - longer interval
        { rating: 5, expectedMinDays: 12, expectedMaxDays: 16 }
        // Very Easy - longest interval
      ];
      for (const testCase of testCases) {
        const mockResult = {
          flashcard_id: "card-123",
          rating: testCase.rating,
          next_review_date: new Date(Date.now() + (testCase.expectedMinDays + 1) * 24 * 60 * 60 * 1e3).toISOString(),
          review_count: 1,
          difficulty_rating: 2.5,
        ...
        const daysDifference = Math.floor((nextReviewDate.getTime() - Date.now()) / (24 * 60 * 60 * 1e3));
    ...
    it("should increase intervals for higher review counts", async () => {
      const firstReview = {
        flashcard_id: "card-123",
        rating: 3,
        next_review_date: new Date(Date.now() + 4 * 24 * 60 * 60 * 1e3).toISOString(),
        review_count: 1,
        difficulty_rating: 2.5,
        session_progress: { cards_reviewed: 1, session_duration_minutes: 5 }
      };
      const fifthReview = {
        flashcard_id: "card-123",
        rating: 3,
        next_review_date: new Date(Date.now() + 6 * 24 * 60 * 60 * 1e3).toISOString(),
        review_count: 5,
        difficulty_rating: 2.5,
        session_progress: { cards_reviewed: 1, session_duration_minutes: 5 }
      };
      const mockLearningService = {
        rateFlashcard: vi.fn().mockResolvedValueOnce(firstReview).mockResolvedValueOnce(fifthReview)
      };
      ...
      const days1 = Math.floor((new Date(result1.next_review_date).getTime() - Date.now()) / (24 * 60 * 60 * 1e3));
      const days5 = Math.floor((new Date(result5.next_review_date).getTime() - Date.now()) / (24 * 60 * 60 * 1e3));
